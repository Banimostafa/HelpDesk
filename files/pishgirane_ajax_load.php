...
include('dbu.class.php');
	...
	$db->setup($user,$pass,$database,$host);
		...
		$query="SELECT * FROM `pishgirane_act` WHERE `status`!='deleted' ";
		...
		$t_num=$n;	$paging='';
			...
			for($i=0;$i<ceil($n/20);$i++) {
				...
				if($i==$page) { 
					...
					$paging.='" onclick="loadpishgirane('.$i.')"';
				}
				$paging.='>'.farsiNum($i+1).'</div>';
			 }
		
		$query="SELECT * FROM `pishgirane_act` WHERE `status`!='deleted' LIMIT ".($page*20).",20  ";
		$result=$db->select($query);
		$j=0;	$rowData='';
		while($row=mysql_fetch_assoc($result)) {
			...
			$rowData.='<span style="width:400px;" id="col-1'.$id.'">'.$row['act_title'].'</span>';
			$rowData.='</div>';

			}
			
 $command="loadpishgirane('".$page."');";
