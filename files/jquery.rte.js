...
content='<br>';var doc="<html><head>"+css+"</head><body style='font-family:tahoma; font-size:12px; cursor:text;' dir='rtl'>"+content+"</body></html>";tryEnableDesignMode(iframe,doc,function(){$("#toolbar-"+iframe.title).remove();$(iframe).before(toolbar(iframe));textarea.remove();});}

function disableDesignMode(iframe,submit){var content=iframe.contentWindow.document.getElementsByTagName("body")[0].innerHTML;
if(submit==true)
...
function toolbar(iframe){var tb=$("<div class='rte-toolbar' id='toolbar-"+iframe.title+"'><div>\
            <p>\
                <a href='#' class='bold'><img src='images/bold.png' alt='bold' /></a>\
                <a href='#' class='italic'><img src='images/italic.png' alt='italic' /></a>\
            </p>\
            <p>\
                <a href='#' class='unorderedlist'><img src='images/unordered.png' alt='unordered list' /></a>\
                <a href='#' class='link'><img src='images/link.png' alt='link' /></a>\
                <a href='#' class='disable'><img src='images/close.png' alt='close rte' /></a>\
            </p></div></div>")
;$('select',tb).change(function(){var index=this.selectedIndex;if(index!=0){var selected=this.options[index].value;formatText(iframe,"formatblock",'<'+selected+'>');}});
$('.bold',tb).click(function(){formatText(iframe,'bold');return false;});
$('.italic',tb).click(function(){formatText(iframe,'italic');return false;});
$('.align-right',tb).click(function(){formatText(iframe,'justifyright');return false;});
$('.unorderedlist',tb).click(function(){formatText(iframe,'insertorderedlist');return false;});
$('.link',tb).click(function(){var p=prompt("URL:");if(p)
formatText(iframe,'CreateLink',p);return false;});
$('.image',tb).click(function(){var p=prompt("image URL:");if(p)
